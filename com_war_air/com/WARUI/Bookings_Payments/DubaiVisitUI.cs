...
{
    public partial class DubaiVisitUI : WarFramework.Forms.WarForm
    {
        long orignalCustomerid;
        long OrignalVisitiDForUpdates;
        ...
        public DubaiVisitUI(long orignalVisitiD )
        {
            InitializeComponent();
            try
            {
                ...
                // TransecLoad
                var tt = WARPRO.Glob.Bookings_And_Payments.Glob_Transec.Get_Transec_Row(orignalVisitiD.ToString());
                PanelStatus.Show();
                ...
                var rr = WARPRO.Glob.CustomersAndAgents.Glob_DubaiVisit.Get_DubaiVisit_Row(orignalVisitiD);
                //'''''''''
                OrignalVisitiDForUpdates = rr.VISIT_ID;

                orignalCustomerid = rr.CUSTOMER_ID;


                ProtectorId_Txt.Text = "VISIT ID : " + rr.VISIT_ID.ToString();
                ProtectorId_Txt.Show();

                Custmer_Or_Agent_ID_Txt.Text = rr.CUSTOMER_ID.ToString();
                VisaDurationDays_Txt.Text = rr.VISA_DURATION_DAYS.ToString();
                IssuedFromTxt.Text= rr.ISSUED_FROM;
        ...
        private void SAVE_NewTicket_Btn_Click(object sender, EventArgs e)
        {
            try
            {
                if (ImageR.Image != null)
                {
                    ...
                    WARPRO.Glob.CustomersAndAgents.Glob_DubaiVisit.NewDubaiVisaInsert(long.Parse(Custmer_Or_Agent_ID_Txt.Text), int.Parse(VisaDurationDays_Txt.Text), IssuedFromTxt.Text,  PasspoetSendingDateTime.Value, PassportRecievingDateTime.Value, decimal.Parse(ChargesTxt.Text), decimal.Parse(PayAbleTxt.Text), decimal.Parse(ProfitTxt.Text));


                    WARPRO.Glob.Bookings_And_Payments.Glob_Transec.NewTransec(WARPRO.Glob.CustomersAndAgents.Glob_DubaiVisit.GetLastID(), Custmer_Or_Agent_ID_Txt.Text, RecievableTxt.Text, T_RecieveAmount.Text, RemainingTxt.Text, "DUBAI VISIT");
        ...
        private void SAVE_Edit_Btn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ImageR.Image != null)
                {
                    ...
                    WARPRO.Glob.CustomersAndAgents.Glob_DubaiVisit.Update_DubaiVisitInfo(long.Parse(Custmer_Or_Agent_ID_Txt.Text), int.Parse(VisaDurationDays_Txt.Text), IssuedFromTxt.Text, PasspoetSendingDateTime.Value, PassportRecievingDateTime.Value, decimal.Parse(ChargesTxt.Text), decimal.Parse(PayAbleTxt.Text), decimal.Parse(ProfitTxt.Text) , OrignalVisitiDForUpdates);
        ...
        private void VisaDurationDays_Txt_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void VisaDurationDays_Txt_KeyPress_1(object sender, KeyPressEventArgs e)
        {
            e.Handled = !Char.IsDigit(e.KeyChar) && e.KeyChar != (char)8 && e.KeyChar != (char)13;
        ...
        private void PayRemainingBtn_Click(object sender, EventArgs e)
        {
            try
            {
                TransecUI tt = new TransecUI(OrignalVisitiDForUpdates.ToString());
                tt.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        private void StatusToggle_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                WARPRO.Glob.Bookings_And_Payments.Glob_Transec.UpdateStatus(StatusToggle.Checked, OrignalVisitiDForUpdates.ToString());
